...
function Register({setToken, setShowRegister}){
    ...
    const handleSubmit = async (e)=>{
        e.preventDefault()
        axios.post(process.env.REACT_APP_API_URL + '/api/auth/register', {username, email, password})
        .then(()=>axios.post(process.env.REACT_APP_API_URL + '/api/auth/login', {email, password}))
        .then(response => {localStorage.setItem('token', response.data.token); setToken(response.data.token)})
    }
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 flex items-center justify-center">
            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-8 w-full max-w-md shadow-2xl">
                <div className="text-center mb-8">
                    ...
                    <p className="text-blue-200 text-sm">Join and start pinning your world.</p>
                </div>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label className="text-blue-200 text-sm font-medium block mb-1">Username</label>
                        <input
                            type="text"
                            value={username}
